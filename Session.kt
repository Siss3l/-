...
import database.DbFactory.init
import database.DbFactory.shut
...
object Session {
    ...
    fun authTCP(host: String, port: Int) {
        val server: TcpServer = TcpServer.create()
            ...
            .doOnConnection { c ->
                ...
                ctx.key = hc.key
                clients[c.channel().id().asLongText()] = ctx
    ...
    private fun sessionHandler(inb: NettyInbound, out: NettyOutbound): Publisher<Void> {
        inb.withConnection { c ->
            c.inbound().receiveObject()
                .subscribe { p ->
                    handlePacket(clients[c.channel().id().asLongText()]!!, p.toString())
                }
            c.onDispose {
    ...
    fun gameTCP(host: String, port: Int) {
        val server: TcpServer = TcpServer.create()
            ...
            .handle(::gameHandler) // onErrorDropped
        server.bindNow().onDispose().block()
    }
}
...
fun start() {
    runBlocking {
        val authJob = CoroutineScope(Dispatchers.Default).async {
            authTCP(Config.host, Config.authPort)
        }
        val gameJob = CoroutineScope(Dispatchers.Default).async {
            gameTCP(Config.host, Config.gamePort)
        }
        authJob.await()
        gameJob.await()
    }
